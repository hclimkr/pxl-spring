...
대용량 다운로드는 `toResponseEntity(...)`보다 `toResponse(...)`를 권한다. `PxlExcelZipExporter`는 두 응답 목적지 모두에서 **아카이브 전체**를 버퍼링하므로 엔트리가 많을수록 차이가 커진다. CSV Exporter 둘은 목적지를 건드리기 전에 코어가 출력을 만들어 두므로 표의 모든 수치 위에 그 렌더링이 더 얹히지만, 얹히는 것은 최대 4 MiB뿐이고 나머지는 Heap이 아니라 임시 파일에 있다.
